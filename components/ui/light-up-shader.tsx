...
const InteractiveStarfieldShader = () => {
    ...
    const [dimmingDisabled, setDimmingDisabled] = useState(true); // Start with dimming DISABLED for visibility
    ...
    `;
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        ...
        for(int i = 0; i < 20; i++) {
          ...
          // Calculate intensity and color - INCREASED BRIGHTNESS 100x
          float len = length(clamped_p - p);
          if (len > 0.0) {
            vec4 star = 0.1 / len * (cos(p.y / 0.1 + vec4(0.0, 1.0, 2.0, 3.0)) + 1.0);
            O += star;
          }
        }
    ...
    `;
        ...
        console.log('✅ Shader and mesh created');
        ...
        <>
            ...
            <div style={{
                ...
                <button style={{ ...buttonStyle, backgroundColor: dimmingDisabled ? '#dc3545' : '#333' }} onClick={() => setDimmingDisabled(!dimmingDisabled)}>
                    {dimmingDisabled ? 'Enable' : 'Disable'} Dimming
                </button>
            </div>
        </>
    );
