...
export const HorizonHeroSection = () => {
    ...
    useEffect(() => {
        const initThree = async () => {
            const { EffectComposer } = await import('three-stdlib');
            const { RenderPass } = await import('three-stdlib');
            const { UnrealBloomPass } = await import('three-stdlib');
        ...
        const animate = () => {
            ...
            refs.mountains.forEach((mountain, i) => {
                ...
                mountain.position.y = 50 + (Math.cos(time * 0.15) * 1 * parallaxFactor); // Added minimal movement
            });
    ...
    useEffect(() => {
        ...
        const elementsToAnimate = [menuRef.current, titleRef.current, subtitleRef.current, scrollProgressRef.current].filter(Boolean);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const cameraPositions = [
                { x: 0, y: 30, z: 300 },    // Section 0 - DHRUV BAJPAI
                { x: 0, y: 40, z: -50 },     // Section 1 - VIT BHOPAL
                { x: 0, y: 50, z: -700 }       // Section 2 - CITY MONTESSORI SCHOOL
            ];
